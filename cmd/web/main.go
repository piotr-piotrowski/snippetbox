...
func main() {
	...
	// and some short help for text explaining what the flag controls.
	...
	// always contain the default value of ":4000".
	...
	// not the value itself. So in the code, that means the addr variable is actually a pointer,
	// and we need to dereference it (i.e prefix it with the * symbol) before using it.
	// Note that we're using the log.Printf() function to interpolate the address with
	// the log message.
